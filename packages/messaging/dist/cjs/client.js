...
__export(client_exports, {
  SuiStackMessagingClient: () => SuiStackMessagingClient,
  messaging: () => messaging
});
...
var import_envelopeEncryption = require("./encryption/envelopeEncryption.js");
var import_creator_cap = require("./contracts/sui_stack_messaging/creator_cap.js");
...
const _SuiStackMessagingClient = class _SuiStackMessagingClient {
  ...
  constructor(options) {
    ...
    if (!options.packageConfig) {
      ...
      switch (network) {
        case "testnet":
          __privateSet(this, _packageConfig, import_constants.TESTNET_MESSAGING_PACKAGE_CONFIG);
          break;
        case "mainnet":
          __privateSet(this, _packageConfig, import_constants.MAINNET_MESSAGING_PACKAGE_CONFIG);
          break;
        default:
          __privateSet(this, _packageConfig, import_constants.TESTNET_MESSAGING_PACKAGE_CONFIG);
          break;
    ...
    const sealApproveContract = __privateGet(this, _packageConfig).sealApproveContract ?? {
      ...
      ...import_constants.DEFAULT_SEAL_APPROVE_CONTRACT
    };
    __privateSet(this, _envelopeEncryption, new import_envelopeEncryption.EnvelopeEncryption({
  ...
  static experimental_asClientExtension(options) {
    return {
      ...
      register: (client) => {
        ...
        if (!sealClient) {
          throw new import_error.MessagingClientError("SealClient extension is required for MessagingClient");
        }
        if (!("storage" in options) && !("walrusStorageConfig" in options)) {
          throw new import_error.MessagingClientError(
        ...
        if (!packageConfig) {
          ...
          switch (network) {
            case "testnet":
              packageConfig = import_constants.TESTNET_MESSAGING_PACKAGE_CONFIG;
              break;
            case "mainnet":
              packageConfig = import_constants.MAINNET_MESSAGING_PACKAGE_CONFIG;
              break;
            default:
              packageConfig = import_constants.TESTNET_MESSAGING_PACKAGE_CONFIG;
              break;
        ...
        const storage = "storage" in options ? (c) => options.storage(c) : (c) => {
          return new import_walrus.WalrusStorageAdapter(c, options.walrusStorageConfig);
        };
  ...
  async getChannelMemberships(request) {
    const memberCapsRes = await __privateGet(this, _suiClient).core.listOwnedObjects({
      ...request,
      type: import_member_cap.MemberCap.name.replace("@local-pkg/sui-stack-messaging", __privateGet(this, _packageConfig).packageId),
    ...
    const memberships = await Promise.all(
      contents.map(async (content) => {
        const parsedMemberCap = import_member_cap.MemberCap.parse(content);
  ...
  async getChannelObjectsByChannelIds(request) {
    const logger = (0, import_logging.getLogger)(import_logging.LOG_CATEGORIES.CLIENT_READS);
    ...
    const parsedChannels = await Promise.all(
      channelObjectsRes.objects.map(async (object) => {
        if (object instanceof Error || !object.content) {
          throw new import_error.MessagingClientError(`Failed to parse Channel object: ${object}`);
        }
        return import_channel.Channel.parse(await object.content);
      })
    );
  ...
  async getCreatorCap(userAddress, channelId) {
    ...
    const creatorCap = await __privateMethod(this, _SuiStackMessagingClient_instances, findOwnedObjectByChannelId_fn).call(this, import_creator_cap.CreatorCap, userAddress, channelId);
    if (creatorCap) {
  ...
  async getUserMemberCap(userAddress, channelId) {
    ...
    const memberCap = await __privateMethod(this, _SuiStackMessagingClient_instances, findOwnedObjectByChannelId_fn).call(this, import_member_cap.MemberCap, userAddress, channelId);
    if (memberCap) {
  ...
  async getChannelMembers(channelNameOrId) {
    ...
    if (channelObject instanceof Error || !channelObject.content) {
      throw new import_error.MessagingClientError(`Failed to parse Channel object: ${channelObject}`);
    }
    const channel = import_channel.Channel.parse(await channelObject.content);
    ...
    for (const obj of memberCapObjects.objects) {
      ...
      try {
        const memberCap = import_member_cap.MemberCap.parse(await obj.content);
        if (obj.owner) {
          let memberAddress;
  ...
  async createChannelFlow({
    ...
    const build = () => {
      ...
      const config = tx.add((0, import_config.none)({ package: pkg }));
      const [channel, creatorCap, creatorMemberCap] = tx.add(
        (0, import_channel._new)({ package: pkg, arguments: { config } })
      );
      ...
      tx.add((0, import_channel.share)({ package: pkg, arguments: { self: channel, creatorCap } }));
      tx.add(
        (0, import_member_cap.transferToRecipient)({
          package: pkg,
      ...
      if (memberCaps !== null) {
        tx.add(
          (0, import_member_cap.transferMemberCaps)({
            package: pkg,
            arguments: {
      ...
      tx.add((0, import_creator_cap.transferToSender)({ package: pkg, arguments: { self: creatorCap } }));
      tx.setGasBudget(5e7);
      return tx;
    };
    ...
    const generateAndAttachEncryptionKey = async ({
      ...
      const tx = new import_transactions.Transaction();
      tx.add(
        (0, import_channel.addEncryptedKey)({
          package: pkg,
          arguments: {
  ...
  async executeSendMessageTransaction({
    ...
    if (messageId === void 0) {
      throw new import_error.MessagingClientError("Message id not found on the transaction effects");
    }
  ...
  async executeAddMembersTransaction({
    ...
    const memberCapsWithOwner = await __privateMethod(this, _SuiStackMessagingClient_instances, getCreatedObjectsByType_fn).call(this, {
      effects,
      objectTypeName: import_member_cap.MemberCap.name,
      parseFunction: (content) => import_member_cap.MemberCap.parse(content),
...
getUserMemberCapId_fn = async function(userAddress, channelId) {
  ...
  if (!memberCap) {
    throw new import_error.MessagingClientError(`User ${userAddress} is not a member of channel ${channelId}`);
  }
...
createAttachmentsVec_fn = async function(tx, encryptedKey, channelId, memberCapId, sender, attachments) {
  const attachmentType = __privateGet(this, _packageConfig).packageId ? (
    ...
    import_attachment.Attachment.name.replace("@local-pkg/sui-stack-messaging", __privateGet(this, _packageConfig).packageId)
  ) : import_attachment.Attachment.name;
...
executeTransaction_fn = async function(transaction, signer, action, waitForTransaction = true) {
  ...
  if (!txn) {
    throw new import_error.MessagingClientError(`Failed to ${action}: no transaction in result`);
  }
...
getGeneratedCaps_fn = async function(digest) {
  ...
  const creatorCapsWithOwner = await __privateMethod(this, _SuiStackMessagingClient_instances, getCreatedObjectsByType_fn).call(this, {
    effects,
    objectTypeName: import_creator_cap.CreatorCap.name,
    parseFunction: (content) => import_creator_cap.CreatorCap.parse(content),
  ...
  const parsedObjectsWithOwner = await Promise.all(
    matchingObjects.map(async (objectResponse) => {
      if (objectResponse instanceof Error || !objectResponse.content) {
        throw new import_error.MessagingClientError(errorMessage);
      }
...
deriveMessageIDsFromRange_fn = function(messagesTableId, startIndex, endIndex) {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    messageIDs.push((0, import_utils.deriveDynamicFieldID)(messagesTableId, "u64", import_bcs.bcs.U64.serialize(i).toBytes()));
  }
  return messageIDs;
};
parseMessageObjects_fn = async function(messageObjects) {
  const DynamicFieldMessage = import_bcs.bcs.struct("DynamicFieldMessage", {
    id: import_bcs.bcs.Address,
    // UID is represented as an address
    name: import_bcs.bcs.U64,
    // the key (message index)
    value: import_message.Message
    // the actual Message
  });
  const parsedMessageObjects = await Promise.all(
    messageObjects.objects.map(async (object) => {
      if (object instanceof Error || !object.content) {
        throw new import_error.MessagingClientError(`Failed to parse message object: ${object}`);
      }
  ...
  if (needsBatchFetch) {
    ...
    const batchContents = await Promise.all(
      objectResponses.objects.map(async (obj) => {
        if (obj instanceof Error || !obj.content) {
          throw new import_error.MessagingClientError(`Failed to fetch object content: ${obj}`);
        }
